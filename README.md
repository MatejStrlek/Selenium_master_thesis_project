...
**Full E2E parity reached: 42/42 tests passing locally across 15 spec files** — every real functional flow the Playwright suite covers (public login, admin/professor/student CRUD and permission checks, cross-role localization and content viewing), matched one-to-one, against the same app and seeded data. **CI needs re-verification**: the run triggered by this milestone's push was still stuck on the test-execution step well past the suite's normal ~1-minute local runtime at last check — not yet root-caused, likely a headless-specific difference (a Bootstrap modal/dropdown visibility wait or the CSV download step are the prime suspects, both new in this milestone and both environment-sensitive). Don't treat CI as confirmed green until that's actually chased down. Visual regression, network-mocking, and pure API tests are deliberately out of scope (see "Why this repo exists"). Next: root-cause the CI hang, then begin collecting the actual comparison metrics — wall-clock execution time against the Playwright suite (same machine, same app state, same scope), a repeated-run stability check (retries off, count first-attempt failures), a setup-complexity tally (dependencies, config, LOC to first green test), and a maintainability/locator-resilience read — now that scope between the two suites is genuinely, fully comparable.
